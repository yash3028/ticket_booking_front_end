...
function AgentSign() {
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const travelName = (e.target as HTMLFormElement).elements.namedItem("travelName") as HTMLInputElement;
        ...
        const agent: User = {
            ...
            user_role: 'agent',
            company_name: travelName.value,  
            date_of_birth: new Date(dob.value),
            password: password.value,
            token: null
        }

        const response = await post_request("http://localhost:3001/users/save-user", agent);
